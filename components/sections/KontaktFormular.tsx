...
export function KontaktFormular({ b, standorte, erster }: { b: KontaktFormularBaustein; standorte: Standort[]; erster: boolean }) {
  ...
  const feld = "mt-1.5 block w-full min-h-12 rounded-sm border-2 border-linie bg-white px-3 py-2.5 text-tinte placeholder:text-grau/70 focus:border-blau focus:outline-none";
  ...
  return (
    <Reveal as="section" className="mx-auto max-w-6xl px-4 py-14 sm:px-6 sm:py-20">
      ...
      <form onSubmit={absenden} className="mt-8 grid max-w-3xl gap-5 sm:grid-cols-2" noValidate={false}>
        ...
        <div className="sm:col-span-2">
          <button type="submit" className="inline-flex min-h-12 cursor-pointer items-center justify-center rounded-sm bg-rot px-6 py-3 font-semibold text-white transition-colors hover:bg-rot-tief">
